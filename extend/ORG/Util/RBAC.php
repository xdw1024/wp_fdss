...
/**
 * RBAC权限认证库类
 * 参照dwz_thinkphp官方框架RBAC编写，未用到方法已注释
 *
 * @package  ORG
 * @author  cx
 * @date  20161116
...
class RBAC {
    ...
    static public function authenticate($map,$model='')
    {
        ...
        return \think\Db::view($model, '*' )->where($map)->find();
    }
     ...
     * @date  20161116
     */
    static function checkAccess($request)
    {
        ...
        if(Config::get('USER_AUTH_ON') ){
            ...
            if((!empty($_module['no']) && !in_array(strtoupper($request->module()),$_module['no'])) || (!empty($_module['yes']) && in_array(strtoupper($request->module()),$_module['yes']))) {
				...
				//检查当前操作是否需要认证
				if((!empty($_action['no']) && !in_array(strtoupper($request->action()),$_action['no'])) || (!empty($_action['yes']) && in_array(strtoupper(ACTION_NAME),$_action['yes']))) {
					return true;
				}else {
					return false;
				}
     ...
     * @date  20161116
     */
    static public function AccessDecision($request)
    {
        //检查是否需要认证
        if (RBAC::checkAccess($request)) {
            //存在认证识别号，则进行进一步的访问决策
            $accessGuid = md5($request->module() . $request->controller() . $request->action());
            if (empty(Session::get(Config::get('ADMIN_AUTH_KEY')))) {
                if (Config::get('USER_AUTH_TYPE') == 2) {
                    //加强验证和即时验证模式 更加安全 后台权限修改可以即时生效
                    //通过数据库进行访问检查
                    $accessList = RBAC::getAccessList(Session::get(Config::get('USER_AUTH_KEY')));
                } else {
                ...
                if (!isset($accessList[strtoupper($request->module())][strtoupper($request->controller())][strtoupper($request->action())])) {
...
//        if(null===$authId)   $authId = $_SESSION[Config::get('USER_AUTH_KEY')];
...
//        if(RBAC::checkAccess()) {
//            //检查认证识别号
//            if(!$_SESSION[Config::get('USER_AUTH_KEY')]) {
//                if(Config::get('GUEST_AUTH_ON')) {
//                    // 开启游客授权访问
//                    if(!isset($_SESSION['_ACCESS_LIST']))
//                        // 保存游客权限
//                        RBAC::saveAccessList(Config::get('GUEST_AUTH_ID'));
//                }else{
//                    // 禁止游客访问跳转到认证网关
//                    redirect(PHP_FILE.Config::get('USER_AUTH_GATEWAY'));
//                }
//            }
//        }
//        return true;
...
//    {
//        // Db方式权限数据
//        $db     =   Db::getInstance(Config::get('RBAC_DB_DSN'));
