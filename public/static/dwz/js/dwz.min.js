	...
	$.extend($.fn, {
		jTree: function(options) {
			var op = $.extend({
				...
				folderExp: "fa fa-folder",//左侧菜单二级目录的文件夹图标（折叠） class
				folderColl:"fa fa-folder-open", //左侧菜单二级目录的文件夹图标（展开） class
		...
		genTree: function(options) {
			...
			return this.each(function() {
				...
				if(tree.size() > 0) {
					...
					$(">div", node).prepend('<div class="node"></div>' + (op.ckbox ? '<div class="ckbox ' + checked + '"></div>' : "") + (op.icon ? '<div class="' + (node.attr("data-icon") || "fa fa-file-text") + '"></div>' : ""));
					addSpace(op.level, node);
					if(op.isLast) {
						$(node).addClass("last")
					}
		...
		_check: function(op) {
			...
			ckbox.append("<input type='checkbox' stispyle='dlay:none;' " + attrs + "/>").click(function() {
...
function initLayout() {
	...
	$("#sidebar, #sidebar_s .collapse, #splitBar, #splitBarProxy").height(iContentH +9);
	$("#taskbar").css({
...
function mydialogAjaxDone(json) {
	DWZ.ajaxDone(json);
	if(json[DWZ.keys.statusCode] == DWZ.statusCode.ok) {
		$("#"+json.reload_dom).loadUrl(json.reload_url,null,null);
		if(json.navTabId) {
			navTab.reload(json.forwardUrl, {
				navTabId: json.navTabId
			})
